window.utils = {
    ...
    setReactValue: async (selector, val) => {
        ...
        // 1. Native value setter로 값 설정
        const nativeInputValueSetter = Object.getOwnPropertyDescriptor(prototype, 'value').set;
        nativeInputValueSetter.call(el, val);

        // 2. React의 value tracker 업데이트
        const tracker = el._valueTracker;
        if (tracker) {
            tracker.setValue(val);
        }

        // 3. 이벤트 발생 (키보드 입력과 동일하게)
        ...
        // 4. React의 onChange handler 직접 호출 (있다면)
        if (el.onchange) {
            el.onchange({ target: el, bubbles: true });
        }
        
        el.blur();
        
        // 5. React 상태 업데이트가 완료될 때까지 대기
